...
//  Compile with Matlab command ">>mex A_mex.c".
//
//  Note: The transposeed matrix is generated.
//
//  Use: A=A_mex(Phi,rho,nx,ny,nz,hx,hy,hz);
//
    ...
    int nrhs, const mxArray     *prhs[])
{
    ...
    int     offset_ux,offset_uy,offset_uz;
...
//     printf("size of ibegin, iend is %d% %d \n",ibegin, iend);
    
    idummy=mxGetM(prhs[0]);
    if(idummy!=(3*nx*ny*nz))
        mexErrMsgTxt("Input argument 1 (Phi) must be a column vector with length 3*nx*ny*nz");
    Phi=mxGetPr(prhs[0]);
...
if (nrhs == 14){    
    nnz=(3*3*2*(nz-2)*(ny-2)*(nx-2))-(3*3*2*(kend-kbegin+1)*(jend-jbegin+1)*(iend-ibegin+1)); // + 3*((2*nx*ny) + (2*(nz-2)*ny)+(2*(nz-2)*(nx-2)));
}
else{
    nnz=(3*3*2*(nz-2)*(ny-2)*(nx-2));
};
...
//             index_data++;            
            ...
            Ajc[index_col]=index_data;
            Air[index_data]=index_zminus;   A[index_data]=-(rho/(2*hz))*Phi[offset_ux+index_zminus];    index_data++;
            Air[index_data]=index_yminus;   A[index_data]=-(rho/(2*hy))*Phi[offset_ux+index_yminus];    index_data++;
            Air[index_data]=index_xminus;   A[index_data]=-(rho/(2*hx))*Phi[offset_ux+index_xminus];    index_data++;
            Air[index_data]=index_xplus;    A[index_data]= (rho/(2*hx))*Phi[offset_ux+index_xplus];     index_data++;
            Air[index_data]=index_yplus;    A[index_data]= (rho/(2*hy))*Phi[offset_ux+index_yplus];     index_data++;
            Air[index_data]=index_zplus;    A[index_data]= (rho/(2*hz))*Phi[offset_ux+index_zplus];     index_data++;
...
//             index_data++;                
            ...
            Ajc[index_col]=index_data;
            Air[index_data]=index_zminus;   A[index_data]=-(rho/(2*hz))*Phi[offset_uy+index_zminus];    index_data++;
            Air[index_data]=index_yminus;   A[index_data]=-(rho/(2*hy))*Phi[offset_uy+index_yminus];    index_data++;
            Air[index_data]=index_xminus;   A[index_data]=-(rho/(2*hx))*Phi[offset_uy+index_xminus];    index_data++;
            Air[index_data]=index_xplus;    A[index_data]= (rho/(2*hx))*Phi[offset_uy+index_xplus];     index_data++;
            Air[index_data]=index_yplus;    A[index_data]= (rho/(2*hy))*Phi[offset_uy+index_yplus];     index_data++;
            Air[index_data]=index_zplus;    A[index_data]= (rho/(2*hz))*Phi[offset_uy+index_zplus];     index_data++;
...
//             index_data++;                
            ...
            Ajc[index_col]=index_data;
            Air[index_data]=index_zminus;   A[index_data]=-(rho/(2*hz))*Phi[offset_uz+index_zminus];    index_data++;
            Air[index_data]=index_yminus;   A[index_data]=-(rho/(2*hy))*Phi[offset_uz+index_yminus];    index_data++;
            Air[index_data]=index_xminus;   A[index_data]=-(rho/(2*hx))*Phi[offset_uz+index_xminus];    index_data++;
            Air[index_data]=index_xplus;    A[index_data]= (rho/(2*hx))*Phi[offset_uz+index_xplus];     index_data++;
            Air[index_data]=index_yplus;    A[index_data]= (rho/(2*hy))*Phi[offset_uz+index_yplus];     index_data++;
            Air[index_data]=index_zplus;    A[index_data]= (rho/(2*hz))*Phi[offset_uz+index_zplus];     index_data++;
            
        };
    };
//     printf("Check 4");   
//     printf("index_data, nnz %d %d \n", index_data, nnz);
    if(index_data!=(nnz))
